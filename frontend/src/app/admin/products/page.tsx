        ...
        {productsQuery.isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {products.map((p) => (
                  <tr key={p._id} className="border-b border-zinc-800/50 last:border-0 hover:bg-zinc-800/30 transition-colors">
                    <td className="py-3 px-5">
                      <div className="flex items-center gap-3">
                        {p.images[0] ? (
                          <img src={p.images[0].url} alt={p.images[0].alt || p.name} className="w-10 h-10 rounded-lg object-cover bg-zinc-800 shrink-0" />
                        ) : (
                        ...
                        <div className="min-w-0">
                          <p className="font-medium text-zinc-200 truncate max-w-[240px]">{p.name}</p>
                          <p className="text-xs text-zinc-500 truncate max-w-[240px]">{p.sku || p.slug}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-5 text-zinc-400">{p.category?.name ?? '—'}</td>
                    <td className="py-3 px-5"><StatusBadge status={p.status} /></td>
                    <td className="py-3 px-5 text-right text-zinc-300 whitespace-nowrap">{formatPrice(p.price)}</td>
                    <td className="py-3 px-5 text-right text-zinc-400 whitespace-nowrap">{p.stock ?? 0}</td>
                    <td className="py-3 px-5">
                      <div className="flex items-center justify-end gap-1">
                        <Link href={`/admin/products/${p._id}/edit`} className="p-2 rounded-lg text-zinc-400 hover:text-zinc-100 hover:bg-zinc-700/60 transition-colors" aria-label={`Edit ${p.name}`}>
      ...
      <ConfirmModal
        ...
        message={
          confirm?.kind === 'archive' ? (
            <>
              <span className="font-medium text-zinc-200">{confirm?.name}</span> will be hidden from the storefront
              immediately. You can restore it later.
            </>
          ) : (
            <>
              <span className="font-medium text-zinc-200">{confirm?.name}</span> will be permanently deleted. This action
              cannot be undone.
            </>
          )
        }
